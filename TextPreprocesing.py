...
def only_regex_filter(column):
    columns = []
    print("Standardize words")
    for i in tqdm(range(len(column))):
        columns.append(preprocess(column[i]))
    return columns
...
def text_cleaning_only_regex(df, labels):
    df['text'] = only_regex_filter(df['text'])
    df = word_count(df, labels)
    df = df[df['word_count'] > 0]
    df.to_csv("cleanedDataOnlyregex.csv", sep="\t")